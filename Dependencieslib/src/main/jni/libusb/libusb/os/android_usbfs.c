...
static int __get_usbfs_fd(struct libusb_device *dev, mode_t mode, int silent) {
    ...
    LOGI("usbfs_path:%s, dev->bus_number:%u, dev->device_address:%u", usbfs_path, dev->bus_number,
         dev->device_address);
    if (usbdev_names)
        snprintf(path, PATH_MAX, "%s/usbdev%u.%u",
                 usbfs_path, dev->bus_number, dev->device_address);
    else
        snprintf(path, PATH_MAX, "%s/%03u/%03u",
                 usbfs_path, dev->bus_number, dev->device_address);
    LOGI("path:%s, errno:%d", path, errno);
...
LOGI("dpriv->fd:%d, device->os_priv:%s", dpriv->fd, device->os_priv);
...
static int _is_usbdev_entry(struct dirent *entry, int *bus_p, int *dev_p) {
    ...
    if (busnum < 0 || busnum > UINT8_MAX || devnum < 0 || devnum > UINT8_MAX) {
        usbi_dbg("invalid usbdev format '%s'", entry->d_name);
        return LIBUSB_SUCCESS;
    }
...
static const char *find_usbfs_path(void) {
    const char *path = USB_DEVTMPFS_PATH;
    const char *ret = NULL;
    DIR *dir;
    struct dirent *entry;
...
static int op_init2(struct libusb_context *ctx, const char *usbfs) {    // XXX
    ...
    LOGI("usbfs_path:%s", usbfs_path);

    if (UNLIKELY(!usbfs_path)) {
        LOGI("could not find usbfs");
    ...
    LOGI("max iso packet length is (likely) %u bytes", max_iso_packet_len);
    ...
    if (sysfs_available == -1) {
        ...
        if (r == 0 && statfsbuf.f_type == SYSFS_MAGIC) {
            LOGD("sysfs is available");
            sysfs_available = 1;
        } else {
            LOGD("sysfs not mounted");
            sysfs_available = 0;
        }
    }
...
static int op_set_option(struct libusb_context *ctx, enum libusb_option option, va_list ap) {
    ...
    if (option == LIBUSB_OPTION_NO_DEVICE_DISCOVERY) {
        ...
        usbi_dbg("no device discovery will be performed");
...
static int _open_sysfs_attr(
    ...
    snprintf(filename, PATH_MAX, SYSFS_DEVICE_PATH "/%s/%s", sysfs_dir, attr);
...
static int __read_sysfs_attr(struct libusb_context *ctx, const char *sysfs_dir,
    ...
    fd = _open_sysfs_attr(ctx, sysfs_dir, attr);
    if (fd < 0)
        return fd;
...
static int sysfs_scan_device(struct libusb_context *ctx, const char *devname) {
    ...
    ret = android_get_device_address(ctx, 0, &busnum, &devaddr, NULL, devname, -1);
...
static int sysfs_get_active_config(struct libusb_device *dev, int *config) {
    struct android_device_priv *priv = usbi_get_device_priv(dev);

    return __read_sysfs_attr(DEVICE_CTX(dev), priv->sysfs_dir, "bConfigurationValue",
                             UINT8_MAX, config);
}
...
int android_get_device_address(struct libusb_context *ctx, int detached,
    ...
    if (!sysfs_available || detached || NULL == sys_name) {
    ...
    r = __read_sysfs_attr(ctx, sys_name, "busnum", UINT8_MAX, &sysfs_attr);
    if (0 > r)
        return r;
    *busnum = (uint8_t) sysfs_attr;

    r = __read_sysfs_attr(ctx, sys_name, "devnum", UINT8_MAX, &sysfs_attr);
    if (0 > r)
        return r;
    *devaddr = (uint8_t) sysfs_attr;

    usbi_dbg("bus=%d dev=%d", *busnum, *devaddr);
    LOGD("bus = %d, dev = %d", *busnum, *devaddr);
...
static int initialize_device(struct libusb_device *dev, uint8_t busnum,
    ...
    if (sysfs_dir) {
        ...
        if (0 == __read_sysfs_attr(ctx, sysfs_dir, "speed", INT_MAX, &speed)) {
            switch (speed) {
                case 1:
    ...
    if (sysfs_dir) {
        fd = _open_sysfs_attr(dev, sysfs_dir, "descriptors");
    } else if (wrapped_fd < 0) {
    ...
    if (UNLIKELY(priv->descriptors_len < LIBUSB_DT_DEVICE_SIZE)) {
...
static int android_get_parent_info(struct libusb_device *dev,
    ...
    {
        struct android_device_priv *priv = usbi_get_device_priv(it);
...
int android_enumerate_device(struct libusb_context *ctx, uint8_t busnum,
    ...
    LOGI("busnum %d devaddr %d session_id %ld",
         busnum, devaddr, session_id);
    ...
    usbi_dbg("allocating new device for %d/%d (session %ld)",
             busnum, devaddr, session_id);
    LOGI("allocating new device for %d/%d (session %ld)",
         busnum, devaddr, session_id);
...
static int sysfs_get_device_list(struct libusb_context *ctx) {
    ...
    if (UNLIKELY(!devices)) {
        LOGD("opendir devices failed errno=%d", errno);
        return LIBUSB_ERROR_IO;
    }
    ...
    while ((entry = readdir(devices))) {
        ...
        if (sysfs_scan_device(ctx, entry->d_name)) {
            LOGD("failed to enumerate dir entry %s", entry->d_name);
            continue;
        }
...
static int android_default_scan_devices(struct libusb_context *ctx) {
     ...
     * adequacy of sysfs and sets sysfs_available.
     */
    if (sysfs_available != 0)
...
static int op_set_device_fd(struct libusb_device *device, int fd) {
    struct android_device_priv *dpriv = usbi_get_device_priv(device);
    dpriv->fd = fd;
    return LIBUSB_SUCCESS;
}

static int initialize_handle(struct libusb_device_handle *handle) {
    ...
    if (hpriv->fd < 0) {
        if (hpriv->fd == LIBUSB_ERROR_NO_DEVICE) {
            ...
            if (usbi_atomic_load(&handle->dev->attached)) {
                ...
                LOGD("open failed with no device, but device still attached");
...
static void op_close(struct libusb_device_handle *dev_handle) {
    struct android_device_handle_priv *hpriv = usbi_get_device_handle_priv(dev_handle);
    if (!hpriv->fd_removed) {
...
static int op_get_configuration(struct libusb_device_handle *handle,
                                int *config) {
    struct android_device_priv *priv = usbi_get_device_priv(handle->dev);
...
static int discard_urbs(struct usbi_transfer *itransfer, int first, int last_plus_one) {
    ...
    struct android_transfer_priv *tpriv = usbi_get_transfer_priv(itransfer);
    struct android_device_handle_priv *dpriv = usbi_get_device_handle_priv(transfer->dev_handle);
    int i, ret = 0;
...
//		return LIBUSB_ERROR_NO_MEM;
    ...
    tpriv->num_retired = 0;
    tpriv->reap_action = NORMAL;
//	if (NULL == tpriv->iso_packet_offset)
//		return LIBUSB_ERROR_NO_MEM;
