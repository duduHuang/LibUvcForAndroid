...
void API_EXPORTED libusb_free_transfer(struct libusb_transfer *transfer) {
    ...
    usbi_dbg("transfer %p", (void *) transfer);
...
static int add_to_flying_list(struct usbi_transfer *transfer) {
    ...
    struct timespec *timeout = &transfer->timeout;
    struct libusb_context *ctx = ITRANSFER_CTX(transfer);
    int r = 0;
    int first = 1;

    calculate_timeout(transfer);
...
int usbi_handle_transfer_completion(struct usbi_transfer *itransfer,
    ...
    usbi_dbg("transfer %p has callback %p",
...
int API_EXPORTED libusb_try_lock_events(libusb_context *ctx) {
    ...
    if (ru) {
        ...
        LOGD("someone else is closing a device");
        return 1;
    }
...
static int handle_event_trigger(struct libusb_context *ctx) {
    ...
    if (ctx->event_flags & USBI_EVENT_EVENT_SOURCES_MODIFIED)
        LOGD("someone updated the event sources");

    if (ctx->event_flags & USBI_EVENT_USER_INTERRUPT) {
        LOGD("someone purposefully interrupted");
        ctx->event_flags &= ~USBI_EVENT_USER_INTERRUPT;
    }

    if (ctx->event_flags & USBI_EVENT_HOTPLUG_CB_DEREGISTERED) {
        LOGD("someone unregistered a hotplug cb");
    ...
    if (ctx->event_flags & USBI_EVENT_DEVICE_CLOSE)
        LOGD("someone is closing a device");

    /* check for any pending hotplug messages */
    if (ctx->event_flags & USBI_EVENT_HOTPLUG_MSG_PENDING) {
        LOGD("hotplug message received");
    ...
    if (ctx->event_flags & USBI_EVENT_TRANSFER_COMPLETED) {
        ...
        __for_each_completed_transfer_safe(&completed_transfers, itransfer, tmp) {
            ...
            if (r) {
                LOGE("backend handle_transfer_completion failed with error %d", r);
                break;
            }
        }
...
static int handle_events(struct libusb_context *ctx, struct timeval *tv) {
    ...
    usbi_mutex_lock(&ctx->event_data_lock);
    if (ctx->event_flags & USBI_EVENT_EVENT_SOURCES_MODIFIED) {
        LOGD("event sources modified, reallocating event data");
...
int
usbi_add_event_source(struct libusb_context *ctx, usbi_os_handle_t os_handle, short poll_events) {
