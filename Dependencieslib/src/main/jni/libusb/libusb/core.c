...
struct libusb_device *usbi_alloc_device(struct libusb_context *ctx,
    ...
    if (UNLIKELY(r)) {
        LOGD("usbi_mutex_init(&dev->lock) failed...");
        free(dev);
        return NULL;
    }
...
int usbi_sanitize_device(struct libusb_device *dev) {
    ...
    if (dev->device_descriptor.bLength != LIBUSB_DT_DEVICE_SIZE ||
        ...
        LOGD("invalid device descriptor");
    ...
    if UNLIKELY(num_configurations > USB_MAXCONFIG)
    {
        ...
        LOGD("too many configurations");
        return LIBUSB_ERROR_IO;
    } else if (0 == num_configurations)
        LOGD("zero configurations, maybe an unauthorized device");
...
ssize_t API_EXPORTED libusb_get_device_list(libusb_context *ctx,
    ...
    if (libusb_has_capability(LIBUSB_CAP_HAS_HOTPLUG)) {
        LOGI("backend provides hotplug support");
        struct libusb_device *dev;
        ...
        LOGI("backend does not provide hotplug support");
...
int API_EXPORTED libusb_open(libusb_device *dev, libusb_device_handle **handle) {
    ...
    LOGI("open (bus/addr)=(%d/%d)", dev->bus_number, dev->device_address);

    if (UNLIKELY(!usbi_atomic_load(&dev->attached))) {
    ...
    if (UNLIKELY(r < 0)) {
        ...
        LOGI("open %d/%d returns %d", dev->bus_number, dev->device_address, r);
        libusb_unref_device(dev);
...
int API_EXPORTED libusb_get_configuration(libusb_device_handle *dev,
    ...
    uint8_t tmp = 0;
    struct libusb_context *ctx = HANDLE_CTX(dev);

    usbi_dbg("");
    if (usbi_backend->get_configuration)
        r = usbi_backend->get_configuration(dev, &tmp);
...
int API_EXPORTED libusb_claim_interface(libusb_device_handle *dev,
    ...
    if (!usbi_atomic_load(&dev->dev->attached)) {
        RETURN(LIBUSB_ERROR_NO_DEVICE,
        int);
    }
...
int API_EXPORTED libusb_set_interface_alt_setting(libusb_device_handle *dev,
    ...
    LOGD("interface %d altsetting %d", interface_number, alternate_setting);
...
int API_EXPORTED libusb_detach_kernel_driver(libusb_device_handle *dev,
    ...
    if (UNLIKELY(!usbi_atomic_load(&dev->dev->attached)))
        return LIBUSB_ERROR_NO_DEVICE;
...
int API_EXPORTED libusb_init(libusb_context **context) {
    return libusb_init_context(context, NULL, NULL, 0);
}
 ...
 * \see libusb_contexts
 */
int API_EXPORTED libusb_init_context(libusb_context **context, const char *usbfs,
    ...
    if (!context && default_context_refcnt > 0) {
    ...
    for (enum libusb_option option = 0; option < LIBUSB_OPTION_MAX; option++) {
        ...
        r = libusb_set_option(ctx, option);
        if (LIBUSB_SUCCESS != r)
            goto err_free_ctx;
    }
    ...
    for (int i = 0; i < num_options; ++i) {
        r = libusb_set_option(ctx, options[i].option, options[i].value.ival);
        if (LIBUSB_SUCCESS != r)
            goto err_free_ctx;
    }
    ...
    usbi_hotplug_init(ctx);

    if (context) {
        *context = ctx;
        if (!usbi_fallback_context) {
            usbi_fallback_context = ctx;
            usbi_dbg("installing new context as implicit default");
            LOGD("installing new context as implicit default");
        }
    }
    usbi_mutex_static_unlock(&default_context_lock);
